...
namespace GTE
{
	...
	*/
	 ...
	 */
	Bool AnimationManager::IsCompatible(SkeletonConstRef skeleton, AnimationConstRef animation) const
	{
	 ...
	 */
	Bool AnimationManager::IsCompatible(SkinnedMesh3DRendererConstRef meshRenderer, AnimationConstRef animation) const
	{
	 ...
	 */
	AnimationPlayerSharedPtr AnimationManager::RetrieveOrCreateAnimationPlayer(SkeletonRef target)
	{
		NONFATAL_ASSERT_RTRN(target.IsValid(), "AnimationManager::RetrieveOrCreateAnimationPlayer -> Target is not valid.", NullAnimationPlayerRef, true);
		...
		if (activePlayers.find(target->GetObjectID()) == activePlayers.end())
		{
			...
			NONFATAL_ASSERT_RTRN(player.IsValid(), "AnimationManager::RetrieveOrCreateAnimationPlayer -> Unable to create player.", NullAnimationPlayerRef, false);
	 ...
	 */
	AnimationPlayerSharedPtr AnimationManager::RetrieveOrCreateAnimationPlayer(SkinnedMesh3DRendererConstRef renderer)
	{
		NONFATAL_ASSERT_RTRN(renderer.IsValid(), "AnimationManager::RetrieveOrCreateAnimationPlayer -> Mesh renderer is not valid.", NullAnimationPlayerRef, true);
