...
{	
	...
	if(LIGHT_TYPE == 1)
	{
		dir = SHADOW_POSITION.w == 0.0 ? vec4(LIGHT_DIRECTION.xyz, 0.0) : SHADOW_POSITION + normalize(vec4(LIGHT_DIRECTION.xyz, 0.0)) * .0005;
	}
	else
	{
		vec4 dirFromLight = vec4(SHADOW_POSITION.xyz - LIGHT_POSITION.xyz, 0.0);
		dir = SHADOW_POSITION.w == 0.0 ? dirFromLight : SHADOW_POSITION + normalize(dirFromLight) * 0.0005;
	}
